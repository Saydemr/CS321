...
requires(IsString<U> && IsContainer<T<U>>)
{
    cout << " level 1: current item is a container with " << container.size() << " items which are:" << endl;
    for (const auto& item : container)
        cout << " " << item << endl;
...
template <template <typename...> typename T, typename U>
auto print_(const T<U>& container)
requires(IsContainer<T<U>> && IsContainer<U> && !IsString<U>)
{
...
int main(int, char* [])
{
    ...
    auto initval2 = std::vector<vector<string>>(1);
    ...
    auto lambda2 = [](auto& container, const auto& item){
        ...
        for(auto i = 0; i < container.size(); ++i)
            if(i==item.size())
                container[i].push_back(item);

        return container;
    };

    auto lambda_filter = [](const auto& item)
    {
        return !(item.size() == 0);
    };
