// S014646 - ABDULLAH SAYDEMIR
// GCC v11.1.0 (Ubuntu 11.1.0-1ubuntu1~20.04)
...
//template<IsCourseContainer T>
//auto print(const T& container, auto title)
//{
//    std::cout << title;
...
requires(IsCourse<T>)
auto print( const std::array<T,size>& array, auto header)
{
    auto end_minus_one = --array.end();
...
auto print(const auto& v, auto title)                                                -> void
{
...
template < template<typename...> typename PrintableType, typename T, typename ...Rest>
auto print(const PrintableType<T,Rest...>& container, auto header)                     -> void
//requires (IsCourse<T>)
{

    auto end_minus_one = --container.end();
    std::cout << header << std::endl;
...
template <typename T>
auto my_swap( T& first, T& second) -> void
{
    auto temp = first;
    first = second;
    second = temp;
...
auto my_sort(T& container, Lambda lambda) -> auto&
{
    for(auto iter = container.begin(); iter != container.end(); ++iter)
    {
        for(auto iter2 = container.begin(); iter2 != container.end(); ++iter2)
            if ( lambda(*iter, *iter2) )
                my_swap(*iter, *iter2);
    }
    return container;
}

auto ascending_by_name = [](Course a, Course b)
{
    return a.name < b.name;
};
auto descending_by_code = [](Course a, Course b)
{
...
int main(int argc, char* argv[])
{
    ...
    auto l = std::array<Course,3>{
        Course{321, "Programming Paradigms"},
        Course{409, "Advanced C++ Programming"},
        Course{423, "Computer Vision"}
    };
